...
public final class TypeMapper {
    ...
    /**
     * Returns a nullable Avro schema with {@code null} as the FIRST alternative
     * in the union, i.e. {@code ["null", <type>]}.
     *
     * <p>The Avro spec requires the default value to match the FIRST type of a
     * union; since every field defaults to {@code null}, null must come first.
     * {@code SchemaBuilder.nullable()} places the concrete type first, which
     * makes {@code .withDefault(null)} fail at runtime.
     */
    public static Schema toAvroFieldSchema(DataType dt) {
        ...
        switch (dt) {
            ...
            case MONEY:
            case COMPLEX_TYPE:
                return Schema.createUnion(List.of(nullSchema, Schema.create(Schema.Type.STRING)));
            case BYTE:
            case INT:
            case LONG:
                return Schema.createUnion(List.of(nullSchema, Schema.create(Schema.Type.INT)));
            case DOUBLE:
            case NUMERIC:
                return Schema.createUnion(List.of(nullSchema, Schema.create(Schema.Type.DOUBLE)));
            case FLOAT:
                return Schema.createUnion(List.of(nullSchema, Schema.create(Schema.Type.FLOAT)));
            case BOOLEAN:
                return Schema.createUnion(List.of(nullSchema, Schema.create(Schema.Type.BOOLEAN)));
            case BINARY:
            case OLE:
                return Schema.createUnion(List.of(nullSchema, Schema.create(Schema.Type.BYTES)));
            default:
                return Schema.createUnion(List.of(nullSchema, Schema.create(Schema.Type.STRING)));
        }
    }
